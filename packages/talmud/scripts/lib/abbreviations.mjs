/** JS mirror of src/lib/sefref/alignment/abbreviations.ts for verify scripts. */
...
function genericAcronymMatch(hbRaw, sefWords, sj) {
  ...
  for (let k = 0; k < MAX_WORDS && sj + k < sefWords.length; k++) {
    words.push(normHe(sefWords[sj + k]));
  }

  const consume = (rem, wIdx) => {
    if (rem === '') return wIdx;
...
export function abbreviationMatches(hbRaw, sefWords, sj) {
  const s = hbRaw.replace(/[֑-ׇ]/g, '').trim();
  const eq = (a, b) => normHe(a) === normHe(b);
  const startsWith = (word, prefix) => normHe(word).startsWith(normHe(prefix));
  ...
  if (bare.length === 1 && /[׳'״"]/.test(s)) {
    ...
    if (forms && sj < sefWords.length) {
      const n = normHe(sefWords[sj]);
      if (forms.some((f) => normHe(f) === n)) return 1;
    }
  }
